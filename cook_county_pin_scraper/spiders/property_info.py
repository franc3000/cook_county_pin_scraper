...
class PropertyInfoSpider(CSVFeedSpider):
    ...
    def parse_pin(self, response):
        ...
        for i in range(0, 5):
            ...
            years[bill_year] = {"year": bill_year, "bill": bill_amount}

            # these are all optional
            bill_exemption = response.xpath(
                '//div[@id="ContentPlaceHolder1_TaxBillInfo_rptTaxBill_Panel5_{}"]/div[@class="pop2Display"]/a/span/text()'.format(
                    i
                )
            ).extract()
            if bill_exemption:
            ...
            not_available = response.xpath(
                '//div[@id="ContentPlaceHolder1_TaxBillInfo_rptTaxBill_Panel6_{}"]/div[@class="pop2Display"]/a/span/text()'.format(
                    i
                )
            ).extract()
            if not_available:
            ...
            divided_pin = response.xpath(
                '//div[@id="ContentPlaceHolder1_TaxBillInfo_rptTaxBill_Panel4_{}"]/div[@class="pop2Display"]/a/span/text()'.format(
                    i
                )
            ).extract()
            if divided_pin:
        ...
        for row in tax_rates_table:
            ...
            cell = {"year": re.sub("[^A-Za-z0-9]+", "", cells[0]), "tax_rate": re.sub("[^A-Za-z0-9.]+", "", cells[1])}
        ...
        for year, attrs in years.items():
            ...
            appeal_not_being_accepted = response.xpath(
                '//*[@id="appealsnotaccepting2{}-button"]/span/text()'.format(year)
            )
